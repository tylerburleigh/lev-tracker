...
Before drafting `what_changed` or `field_change_note`, review `research/backlog/field-activity-watchlist.v1.json` and the field-activity counts printed by `npm run state-of-field:status`. Resolve or explicitly defer `capture_now`, `research_more`, pending `field_anchor`, and pending `material_program` candidates for the covered period. If new company, funder, prize, partnership, regulator, or trial-watch activity is found, use `docs/field-activity-workflow.md` before adding public activity records.
