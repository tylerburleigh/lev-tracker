...
Before asking the curator to approve monthly State of the Field direction, run `npm run state-of-field:dates -- --write` and `npm run state-of-field:prep -- --write`. Treat `extra/state-of-field-<edition>-prep.md` as the agent synthesis packet: it should show recommended buckets for field signals, current context, trial horizon, omitted/process items, historical context, field-activity lenses, trial-watch inputs, publication-gate blockers, and any approvals that still need human attention. Use `npm run state-of-field:packet` only for unresolved approval decisions; do not ask the curator to classify raw updates from scratch.

Monthly State of the Field placement is based on when the field event happened, not when the tracker added it. For direct field activity, use `activity_item.occurred_on` as the period date and treat `publication_event.published_at` only as audit metadata. For candidate-bundle updates, inspect the date audit's source, study, finding, and trial-date signals; if no single field-event date is defensible, use the tracker publication date as the workflow date basis and describe the item as reviewed context rather than current-period field movement. `npm run state-of-field:reconcile -- --write` records this in each reconciliation item's `date_basis_review`, and the prep packet flags any field-signal or undecided update that still needs agent date judgment before final monthly copy. Late-reviewed items from a prior covered period can revise that older edition only as context unless the period conclusion would have changed.

When a state-of-field edition is not finished, update the workflow manifest instead of adding provisional fields to the public edition schema. Each reconciliation item should point to the public update being classified and record the agent recommendation, approval state, final decision, and event-date basis. Current-period decisions should distinguish `include_as_field_signal`, `include_as_current_context`, `include_as_trial_horizon`, and `omit_process_context`; prior-period decisions should distinguish no-op, deferral, late-reviewed context, and material correction.
