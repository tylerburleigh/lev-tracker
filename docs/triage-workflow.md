...
The generated `field_activity` item is a monthly cross-field sweep. It is suppressed for the current month only after a `research_session` with `mode: "field_activity"` and `field_activity_sweep_type: "monthly_cross_field"` has been recorded. Entity, funder/prize, government-program, registry/regulatory, threshold-audit, and trial-policy sessions do not close the monthly sweep. When the item appears, start from `research/backlog/field-activity-watchlist.v1.json` and run `npm run field-activity:status` before broad discovery. Report publication-ready items, human approval required, source-work candidates, nonblocking watch items, consolidated items, State-of-Field-routed items, and missing surface routing rather than treating all source-work holds as approval blockers.
